...
object DataCleanup {
  ...
  def mergeMaps(values: Seq[Map[String, Int]], threshold: Int): Int = {
    values.reduce((map1, map2) => {
      (map1.keySet ++ map2.keySet).map { key => (key, map1.getOrElse(key, 0) + map2.getOrElse(key, 0)) }.toMap
    }).filter { case (_, score) => score >= threshold }
      .keys
      .toList
      .map(_.toInt).sorted.headOption.getOrElse(0)
  }
